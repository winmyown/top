...
import org.top.java.netty.source.util.internal.shaded.org.jctools.queues.MpscArrayQueue;
import org.top.java.netty.source.util.internal.shaded.org.jctools.queues.MpscChunkedArrayQueue;
import org.top.java.netty.source.util.internal.shaded.org.jctools.queues.MpscUnboundedArrayQueue;
import org.top.java.netty.source.util.internal.shaded.org.jctools.queues.SpscLinkedQueue;
import org.top.java.netty.source.util.internal.shaded.org.jctools.queues.atomic.MpscAtomicArrayQueue;
import org.top.java.netty.source.util.internal.shaded.org.jctools.queues.atomic.MpscChunkedAtomicArrayQueue;
import org.top.java.netty.source.util.internal.shaded.org.jctools.queues.atomic.MpscUnboundedAtomicArrayQueue;
import org.top.java.netty.source.util.internal.shaded.org.jctools.queues.atomic.SpscLinkedAtomicQueue;
import org.top.java.netty.source.util.internal.shaded.org.jctools.util.Pow2;
import org.top.java.netty.source.util.internal.shaded.org.jctools.util.UnsafeAccess;
...
import static org.top.java.netty.source.util.internal.PlatformDependent0.*;
...
/**
 * 用于检测当前运行时环境特定属性的实用工具，例如Java版本和
 * {@code sun.misc.Unsafe}对象的可用性。
 * <p>
 * 如果你指定了系统属性 <strong>io.netty.noUnsafe</strong>，
 * 可以禁用对 {@code sun.misc.Unsafe} 的使用。
 */
public final class PlatformDependent {
    ...
    // 通过 os.detection.classifierWithLikes 保持与 Maven 的 pom.xml 同步！
    ...
    /**
 * 对于规范，请参见 https://www.freedesktop.org/software/systemd/man/os-release.html
 */
    ...
    static {
        ...
        // * <  0  - 不使用清理器，并从 Java 继承最大直接内存。在这种情况下，
        //           "practical max direct memory" would be 2 * max memory as defined by the JDK.
        //           "实际最大直接内存" 为 JDK 定义的 max memory 的 2 倍。
        // * == 0  - Use cleaner, Netty will not enforce max memory, and instead will defer to JDK.
        // * == 0  - 使用更清洁的方式，Netty 不会强制执行最大内存限制，而是会交由 JDK 处理。
        // * >  0  - Don't use cleaner. This will limit Netty's total direct memory
        // * >  0  - 不要使用清理器。这将限制Netty的总直接内存
        ...
        if (!isAndroid()) {
            ...
            /**
 * 参见 https://github.com/netty/netty/issues/2604
 */
            if (javaVersion() >= 9) {
        ...
        // 如果我们可以使用 Cleaner 来释放直接缓冲区，那么默认情况下应该始终优先使用直接缓冲区。
        ...
        /*
         * 我们不希望在明确禁用 unsafe 的情况下记录此消息。不要移除显式的 no unsafe 防护。
         */
                ...
                public Boolean run() {
                    try {
                        if (file.exists()) {
                            ...
                            // 规范指出，只有在 /etc/os-release 不存在时，我们才应回退
                            return true;
                        }
                    } catch (SecurityException e) {
    ...
    static boolean addPropertyOsClassifiers(Set<String> allowedClassifiers, Set<String> availableClassifiers) {
        ...
        // 空: -Dio.netty.osClassifiers (没有特定发行版的本地库分类器)
        // single ID: -Dio.netty.osClassifiers=ubuntu
        // 单ID: -Dio.netty.osClassifiers=ubuntu
        ...
        if (osClassifiers.isEmpty()) {
            ...
            // 允许用户仅使用 -Dio.netty.osClassifiers 省略分类器
            return true;
        }
    ...
    /**
     * 当且仅当当前平台是 Android 时返回 {@code true}
     */
    ...
    /**
     * 如果 JVM 运行在 Windows 上，则返回 {@code true}
     */
    ...
    /**
     * 返回 {@code true} 如果 JVM 运行在 OSX / MacOS 上
     */
    ...
    /**
     * 返回 {@code true} 如果当前用户可能是超级用户。请注意，这只是一个提示，因此可能会返回假阳性。
     */
    ...
    /**
     * 当且仅当默认情况下启用 TCP_NODELAY 套接字选项是合适的时，返回 {@code true}。
     */
    ...
    /**
     * 如果 {@code sun.misc.Unsafe} 在类路径上找到并且可以用于加速直接内存访问，则返回 {@code true}。
     */
    ...
    /**
     * 返回 {@code sun.misc.Unsafe} 不可用的原因（如果有）。
     */
    ...
    /**
     * 当且仅当平台支持非对齐访问时为 {@code true}。
     *
     * @see <a href="https://en.wikipedia.org/wiki/Segmentation_fault#Bus_error">Wikipedia 关于段错误的介绍</a>
     */
    ...
    /**
     * 如果平台具有可靠的低级别直接缓冲区访问 API 并且用户未指定 {@code -Dio.netty.noPreferDirect} 选项，则返回 {@code true}。
     */
    ...
    /**
     * 为当前平台创建一个新的最快的 {@link ConcurrentMap} 实现。
     */
    ...
    /**
     * 尝试释放指定的直接 {@link ByteBuffer}。请注意，如果当前平台不支持此操作或指定的缓冲区不是直接缓冲区，则此方法不执行任何操作。
     */
    ...
    /**
     * 与 {@link PlatformDependent0#hashCodeAsciiCompute(long, int)} 相同，但适用于 {@link CharSequence}。
     */
    ...
    /**
     * 与 {@link PlatformDependent0#hashCodeAsciiSanitize(int)} 相同，但适用于 {@link CharSequence}。
     */
    private static int hashCodeAsciiSanitizeInt(CharSequence value, int offset) {
        if (BIG_ENDIAN_NATIVE_ORDER) {
            // mimic a unsafe.getInt call on a big endian machine
            // 模拟在大端序机器上调用unsafe.getInt
    ...
    /**
     * 与 {@link PlatformDependent0#hashCodeAsciiSanitize(short)} 相同，但适用于 {@link CharSequence}。
     */
    private static int hashCodeAsciiSanitizeShort(CharSequence value, int offset) {
        if (BIG_ENDIAN_NATIVE_ORDER) {
            // mimic a unsafe.getShort call on a big endian machine
            // 模拟在大端机器上调用 unsafe.getShort
    ...
    /**
     * 与 {@link PlatformDependent0#hashCodeAsciiSanitize(byte)} 相同，但适用于 {@link CharSequence}。
     */
    ...
    /**
     * 分配一个具有给定 {@code capacity} 的新 {@link ByteBuffer}。通过此方法分配的 {@link ByteBuffer} <strong>必须</strong> 通过 {@link #freeDirectNoCleaner(ByteBuffer)} 进行释放。
     */
    ...
    /**
     * 使用给定的 {@code capacity} 重新分配一个新的 {@link ByteBuffer}。使用此方法重新分配的 {@link ByteBuffer} <strong>必须</strong> 通过 {@link #freeDirectNoCleaner(ByteBuffer)} 进行释放。
     */
    ...
    /**
     * 此方法<strong>必须</strong>仅用于通过 {@link #allocateDirectNoCleaner(int)} 分配的 {@link ByteBuffer}。
     */
    ...
    /**
     * 比较两个 {@code byte} 数组是否相等。出于性能考虑，不会对参数进行边界检查。
     *
     ...
     * 判断数组的一个子段是否为零。
     * @param bytes 字节数组。
     * @param startPos {@code bytes}中的起始索引（包含）。
     * @param length 要检查的字节数量。
     * @return 如果 {@code bytes[startPos:startsPos+length)} 包含非零值，则返回 {@code false}。
     */
    ...
    /**
     * 计算一个字节数组的哈希码，假设使用ASCII字符编码。
     * 生成的哈希码将不区分大小写。
     * @param bytes 包含要哈希的数据的数组。
     * @param startPos 在 {@code bytes} 中开始生成哈希码的索引。
     * @param length 计算中应考虑的字节数。
     * @return {@code bytes} 的哈希码，假设使用ASCII字符编码。
     * 生成的哈希码将不区分大小写。
     */
    ...
    /**
     * 计算假设使用ASCII字符编码的字节数组的哈希码。
     * 生成的哈希码将不区分大小写。
     * <p>
     * 该方法假设 {@code bytes} 等同于使用 {@link CharSequence} 存储的 {@code byte[]}。
     * 每个 {@code char} 的最高字节将被忽略。
     * @param bytes 包含要哈希的数据的数组（假设等同于 {@code byte[]}）。
     * @return 假设使用ASCII字符编码的 {@code bytes} 的哈希码。
     * 生成的哈希码将不区分大小写。
     */
    public static int hashCodeAscii(CharSequence bytes) {
        ...
        // 性能损失（仅达到遍历每个字符的循环性能的约60%）。因此因为
        // of this we take special provisions to unroll the looping for these conditions.
        // 对此我们采取特殊措施来展开这些条件的循环。
        if (length >= 32) {
    ...
    private static final class Mpsc {
        ...
        static <T> Queue<T> newMpscQueue(final int maxCapacity) {
            ...
            // 计算最大容量，该容量不能超过 MAX_ALLOWED_MPSC_CAPACITY。
            // This is forced by the MpscChunkedArrayQueue implementation as will try to round it
            // 这是由 MpscChunkedArrayQueue 实现强制的，因为它会尝试将其四舍五入
    ...
    /**
     * 创建一个新的{@link Queue}，该队列可以安全地用于多个生产者（不同线程）和单个消费者（一个线程！）。
     * @return 一个可能是无界的MPSC队列。
     */
    ...
    /**
     * 创建一个新的{@link Queue}，该队列可安全地用于单生产者（一个线程！）和单消费者（一个线程！）。
     */
    ...
    /**
     * 创建一个新的 {@link Queue}，该队列可以安全地用于多个生产者（不同线程）和单个消费者（一个线程！），并具有给定的固定 {@code capacity}。
     */
    ...
    /**
     * 返回给定 {@link Class} 的 {@link ClassLoader}。
     */
    ...
    /**
     * 返回当前 {@link Thread} 的上下文 {@link ClassLoader}。
     */
    ...
    /**
     * 返回一个非线程安全的 {@link Random}，因此只能在同一个线程中使用。
     */
    ...
    private static boolean maybeSuperUser0() {
        ...
        // 检查 root 和 toor，因为一些 BSD 系统有一个 toor 用户，基本上与 root 相同。
    ...
    /**
     * 如果运行的 JVM 是 <a href="https://developer.ibm.com/javasdk/">IBM J9</a> 或
     * <a href="https://www.eclipse.org/openj9/">Eclipse OpenJ9</a>，则返回 {@code true}，否则返回 {@code false}。
     */
    ...
    /**
     * 返回 {@code true} 如果当前运行的 JVM 是 <a href="https://www.ikvm.net">IKVM.NET</a>，否则返回 {@code false}。
     */
     ...
     * <p>
     * 该计算不会被缓存，因此你可能希望使用 {@link #maxDirectMemory()} 代替。
     * <p>
     * 调用时将产生调试日志输出。
     *
     * @return 估计的最大直接内存，以字节为单位。
     */
    public static long estimateMaxDirectMemory() {
        ...
        try {
            ...
            // 使用 IBM J9 / Eclipse OpenJ9 时，不应使用 VM.maxDirectMemory()，因为它不能准确反映
            // correct value.
            // 正确值
            // See:
            // 参见：
...
// 问题描述：
// 当使用 `HttpObjectAggregator` 时，如果客户端发送的请求体过大，Netty 会抛出 `TooLongFrameException`。
// 但是，`HttpObjectAggregator` 在处理 `TooLongFrameException` 时，没有正确地关闭连接，导致连接泄漏。

// 重现步骤：
// 1. 配置一个 `HttpObjectAggregator`，设置 `maxContentLength` 为一个较小的值。
// 2. 发送一个请求体超过 `maxContentLength` 的 HTTP 请求。
// 3. 观察连接是否被正确关闭。

// 预期行为：
// 当 `TooLongFrameException` 被抛出时，连接应该被正确关闭，避免连接泄漏。
...
// 连接没有被正确关闭，导致连接泄漏。
            ...
            if (!vmName.startsWith("ibm j9") &&
                    ...
                    /*
                     * 以下宏定义了供应商版本号。
                     * 格式为：<供应商>_<主版本号>.<次版本号>.<维护版本号>_<构建号>
                     #define VENDOR_VERSION "Eclipse OpenJ9 0.8.0"
                     */

                    !vmName.startsWith("eclipse openj9")) {
                // Try to get from sun.misc.VM.maxDirectMemory() which should be most accurate.
                // 尝试从 sun.misc.VM.maxDirectMemory() 获取，这应该是最准确的。
        ...
        try {
            ...
            // 现在尝试获取JVM选项 (-XX:MaxDirectMemorySize) 并解析它。
            // Note that we are using reflection because Android doesn't have these classes.
            // 注意，我们使用反射是因为 Android 没有这些类。
    ...
    /**
     * 仅将存在于 <tt>allowed</tt> 中的分类器字符串添加到 <tt>dest</tt>。
     *
     * @param allowed          允许的分类器
     * @param dest             目标集合
     * @param maybeClassifiers 可能添加的分类器
     */
