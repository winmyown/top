 ...
 * 该生成器可在 jctools-build 模块中找到。原始源文件为 BaseMpscLinkedArrayQueue.java。
 */
 ...
 * 该生成器可在 jctools-build 模块中找到。原始源文件为 BaseMpscLinkedArrayQueue.java。
 *
 * 一个 MPSC 数组队列，初始容量为 <i>initialCapacity</i>，并以初始大小的链接块增长到 <i>maxCapacity</i>。
 * 队列仅在当前缓冲区满时增长，并且在调整大小时不会复制元素，而是在旧缓冲区中存储指向新缓冲区的链接，供消费者跟随。
 */
abstract class BaseMpscLinkedAtomicArrayQueue<E> extends BaseMpscLinkedAtomicArrayQueueColdProducerFields<E> implements MessagePassingQueue<E>, QueueProgressIndicators {
    ...
    public int fill(Supplier<E> s) {
        ...
        // result 是一个 long 类型，因为我们希望定期进行 safepoint 检查
        long result = 0;
    ...
    /**
     * 注意：该类由 io.netty.util.internal.shaded.org.jctools.queues.atomic.JavaParsingAtomicLinkedQueueGenerator
     * 自动生成，该生成器可以在 jctools-build 模块中找到。原始源文件为 BaseMpscLinkedArrayQueue.java。
     */
