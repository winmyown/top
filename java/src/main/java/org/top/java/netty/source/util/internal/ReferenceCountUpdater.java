...
import org.top.java.netty.source.util.IllegalReferenceCountException;
import org.top.java.netty.source.util.ReferenceCounted;
...
public abstract class ReferenceCountUpdater<T extends ReferenceCounted> {
     ...
     *
     * 对于更新的int字段：
     *   偶数 => "实际"引用计数为 (refCnt >>> 1)
     *   奇数 => "实际"引用计数为 0
     *
     * (x & y) 相对于 (x == y) 来说似乎出人意料地昂贵。因此，在检查存活（偶数）引用计数时，此类在某些地方为最常见的低值使用了快速路径，
    ...
    /**
     * 类似于 {@link #realRefCnt(int)}，但如果 refCnt == 0 则抛出异常
     */
    ...
    private int nonVolatileRawCnt(T instance) {
        ...
        // TODO: 一旦我们编译针对更高版本的Java，就可以用varhandles替换这里的Unsafe使用。
        final long offset = unsafeOffset();
