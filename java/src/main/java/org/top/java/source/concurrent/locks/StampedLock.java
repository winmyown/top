...
public class StampedLock implements java.io.Serializable {
     ...
     * 算法注释：
     *
     * 该设计采用了序列锁的元素（如在 Linux 内核中使用的序列锁，参见 Lameter 的
     * http://www.lameter.com/gelato2005.pdf 以及其他地方的参考文献；
     * 参见 Boehm 的 http://www.hpl.hp.com/techreports/2012/HPL-2012-68.html）、
     * 有序读写锁（参见 Shirako 等人 http://dl.acm.org/citation.cfm?id=2312015）。
     *
     * 从概念上讲，锁的主要状态包括一个序列号，该序列号在写锁定时为奇数，否则为偶数。
     * 但这被一个非零的读者计数所抵消，当读锁定时，该计数为非零。在验证“乐观”序列锁读者风格的标记时忽略该计数。
     * 由于我们必须使用有限数量的比特（当前为 7 个）来表示读者，因此当读者数量超过计数字段时，
     * 使用一个补充的读者溢出位来处理。我们通过将最大读者计数值（RBITS）作为保护溢出更新的自旋锁来实现这一点。
     *
     * 等待者使用了 CLH 锁的修改形式，该锁在 AbstractQueuedSynchronizer 中使用
     ...
     * 但在 acquireRead 和 acquireWrite 方法中，其它无更好任务的线程可以帮助完成。
     *
     * 这些规则适用于实际排队的线程。所有 tryLock 形式不顾优先级规则尝试获取锁，因此可能“闯入”。
     ...
     * 则它会增加自旋次数（最多 MAX_HEAD_SPINS），以减少不断输给闯入线程的可能性。
     *
     * 几乎所有这些机制都在 acquireWrite 和 acquireRead 方法中执行，典型地，
     * 此类代码由于行为和重试依赖于一组一致的本地缓存读取，因此非常冗长。
     *
     * 正如 Boehm 的论文（见上文）所述，序列验证（主要是 validate() 方法）需要比普通易失性读取（状态）更严格的排序规则。
     * 为了在这些情况中强制执行读取操作的顺序并进行验证，我们使用了 Unsafe.loadFence。
     *
     * 内存布局将锁状态和队列指针保持在一起（通常在同一个缓存行上）。这通常适用于主要是读操作的负载。
     * 在大多数其他情况下，自适应自旋 CLH 锁减少了内存争用的自然趋势，减少了进一步分散争用位置的动机，但可能会在未来有所改进。
     */
